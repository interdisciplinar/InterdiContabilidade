			...
			if (usuarioDao.verificaUsuario(loginUsuario)) {
				if (bloqueiacampo) {
					...
					FacesContext.getCurrentInstance().addMessage(null,
							new FacesMessage(FacesMessage.SEVERITY_ERROR, "Usuario já foi cadastrado", "Usuário já foi cadastrado"));
				}
			} else {
				if (usuarioDao.setIncluir(cadUsuario)) {
					FacesContext.getCurrentInstance().addMessage(null, new FacesMessage(FacesMessage.SEVERITY_INFO, "Info", "Usuário Adicionado!"));
					getUsuarios();
					refresh();
				} else {
					FacesContext.getCurrentInstance().addMessage(null,
