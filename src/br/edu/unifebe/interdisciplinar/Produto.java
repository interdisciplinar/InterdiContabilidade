	...
	public boolean validaCampoCodProdutos() {
		try {
			...
			if (produtosDao.validaCodProduto(codProduto)) {
				FacesContext.getCurrentInstance().addMessage(null,
						new FacesMessage(FacesMessage.SEVERITY_ERROR, "Codigo do Produto já está cadastrado!", null));
				blockBtn = true;
				return false;
			} else {
				blockBtn = false;
	...
	public boolean validaCampoNomeProdutos() {
		try {
			...
			if (produtosDao.validaNomeProduto(nomeProduto)) {
				FacesContext.getCurrentInstance().addMessage(null,
						new FacesMessage(FacesMessage.SEVERITY_ERROR, "Nome do Produto já está cadastrado!", null));
				blockBtn = true;
				return false;
			} else {
				blockBtn = false;
